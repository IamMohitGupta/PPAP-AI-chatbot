...
if not current_case:
    ...
    **Security:**
    ...
    with tab7:
        ...
        st.markdown(f"""
        Ask questions about this PPAP case or get guidance on PPAP requirements.
        **Context:** Part {current_case['part_number']} Rev {current_case['revision']}
        """)
        ...
        with chat_container:
            for message in st.session_state.chat_history[case_id]:
                with st.chat_message(message['role']):
                    st.markdown(message['content'])

        # Chat input
        user_input = st.chat_input("Ask a question about this PPAP case...")

        if user_input:
            # Add user message
            st.session_state.chat_history[case_id].append({
                'role': 'user',
                'content': user_input
            })
            ...
            # Mock AI response
            mock_responses = {
                'checklist': f"Based on the uploaded documents for {current_case['part_number']}, the PPAP checklist shows 18 out of 24 requirements satisfied. The main gaps are in process capability studies and SPC documentation. Would you like me to explain what's needed for these items?",
                'fair': "The FAIR analysis shows all dimensions are within specification. However, the Inner Diameter (Balloon ID #2) is using 40% of the tolerance range. I recommend monitoring this dimension closely during production runs.",
                'oq': "The OQ documentation is partially complete. You're missing maintenance schedules and calibration certificates. These are critical for FDA compliance. Would you like guidance on what these documents should contain?",
                'pq': f"The PQ validation for {current_case['part_number']} is strong - all Cpk values exceed 1.33, and your production run of 50 units exceeds the minimum requirement of 30. The first pass yield of 98% is excellent for medical device manufacturing.",
                'default': f"I can help you with questions about the PPAP documentation for {current_case['part_number']} Rev {current_case['revision']}. I have access to all uploaded documents, analysis results, and PPAP requirements. What would you like to know?"
            }

            # Determine response based on keywords
            response_key = 'default'
            if 'checklist' in user_input.lower() or 'gap' in user_input.lower():
                response_key = 'checklist'
            elif 'fair' in user_input.lower() or 'dimension' in user_input.lower():
                response_key = 'fair'
            elif 'oq' in user_input.lower() or 'qualification' in user_input.lower():
                response_key = 'oq'
            elif 'pq' in user_input.lower() or 'performance' in user_input.lower():
                response_key = 'pq'

            ai_response = mock_responses[response_key]
        ...
        with col1:
            if st.button("What gaps remain?"):
                st.session_state.chat_history[case_id].append({'role': 'user', 'content': 'What gaps remain in the PPAP checklist?'})
                st.rerun()

        with col2:
            if st.button("Explain Cpk requirements"):
                st.session_state.chat_history[case_id].append({'role': 'user', 'content': 'Can you explain the Cpk requirements for PPAP?'})
                st.rerun()

        with col3:
            if st.button("Next steps?"):
                st.session_state.chat_history[case_id].append({'role': 'user', 'content': 'What are the next steps for this PPAP case?'})
                st.rerun()
